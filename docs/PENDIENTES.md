...
| 14 | 🔴 **El bucle de reenvio de 8 s: la causa real esta en la lectura HTTP de la libreria** | `readHTTPAnswer()` corta apenas leyo el PRIMER bloque disponible (`if (responseReceived) break;`), asi que si los encabezados llegan en un segmento TLS y el cuerpo en el siguiente, se queda con los encabezados y el cuerpo vacio; `checkForOkResponse()` no puede confirmar y `sendPostMessage()` reenvia 8 s. Es una carrera, por eso pasa "a veces" y empeora con mensajes largos. **Medido el 2026-08-20: falla hasta con un mensaje de 486 bytes**, asi que acortar los mensajes NO es solucion. `sendSimpleMessage()` tiene el mismo bucle: no hay salida por la API de la libreria. La solucion propuesta es que el ENVIO deje de pasar por la libreria — una funcion propia que arma el POST, pide `Connection: close`, lee hasta que el server cierra y **no reintenta nunca** (un fallo se registra, jamas se duplica). `getUpdates()` seguiria siendo de la libreria | **espera el OK de Mariano** |
